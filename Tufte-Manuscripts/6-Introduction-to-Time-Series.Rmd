...
```{r, fig.width = 5, fig.height = 2, fig.cap = "The corresponding time series plot or time series graph of Monthly Sales Volume.", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE, fig.fullwidth = TRUE}
## Page 3, create plot
 # First, gather the time series data table into one column.
colnames(table_61) <- rep(c("Period", "Year"), 4)

table_61_series <- rbind(table_61[ ,1:2], table_61[ ,3:4], table_61[ ,5:6], table_61[ ,7:8])
...
ggdraw(figure_61 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), axis.title=element_blank()))
```
...
```{r, fig.fullwidth = TRUE, fig.width = 6, fig.height = 2.6, fig.cap = "Horizontal Data.", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
## Page 3, create plot
# First, gather the time series data table into one column.
table_62_series <- rbind(table_62[,1:2], table_62[,3:4])
...
ggdraw(figure_621 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), axis.title=element_blank()))
```
...
```{r, fig.fullwidth = TRUE, fig.width = 6,fig.height = 2.6, fig.cap = "Horizontal Data with Plotted Mean.", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
ggdraw(figure_622 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), axis.title=element_blank()))
```
...
colnames(table_63_chart) <- c("Period", " ", " ")

table_63_chart <- rbind(table_63_chart[,1:2], table_63_chart[,c(1,3)])

table_63_chart <- data.frame(Period = table_63_chart[,1],
                             Series = c(rep("Actual", 30), rep("CMA(3)",30)),
                             Horizontal_Data = table_63_chart[,2])

chart_63 <- ggplot(data = table_63_chart, aes(y = Horizontal_Data, x = Period, color = Series)) +
            geom_line() + scale_color_manual(values = Picasso) +
            scale_y_continuous(breaks=seq(300,400,10), limits = c(300,400), sec.axis = dup_axis()) +
            scale_x_continuous(breaks=seq(0,35,5), limits = c(0,35)) 
        
chart_63 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank())
```
...
#ggdraw(switch_axis_position(chart_63 + ggtitle("Centered Moving Average(3)") + theme(panel.background = element_rect(fill = "white", colour = "grey50") +  theme(legend.position = "top"), axis = 'y'))
```
...
table_64_series <- data.frame(table_62_series, 
                              "CMA(3)" = smooth_63, 
                              "CMA(5)" = smooth_64)
...
colnames(table_641_chart) <- c("Period", " ", " ")

table_63_chart <- rbind(table_641_chart[,1:2], table_641_chart[,c(1,3)])

table_63_chart <- data.frame(Period = table_641_chart[,1],
                             Series = c(rep("Actual", 30), rep("CMA(5)",30)),
                             Value = table_641_chart[,2])
...
table_642_chart <- data.frame(Period=1:90, 
                              Series=c(rep("Actual", 30), rep("CMA(3)",30),rep("CMA(5)",30)),
                              Value = c(table_62_series[,2], smooth_63, smooth_64))
...
ggdraw(chart_642 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
# table for chart
table_65_chart <- data.frame(Period = 1:60,
                             Series = c(rep("Actual", 30), rep("CMA(4)",30)),
                             Value = c(table_65$Actual, table_65$`CMA(4)`))
...
ggdraw(chart_65 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
\medskip
...
ggdraw(chart_66 + theme(panel.background = element_rect(fill = "white", colour = "grey50") , legend.position = "top", axis.title=element_blank()))
```
...
ggdraw(chart_67 + theme(panel.background = element_rect(fill = "white", colour = "grey50") , legend.position = "top", axis.title=element_blank()))
```
...
ggdraw(figure_610 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
colnames(table_611_series) <- c("Period", "Series", "Sales")
 ...
 cols <- c("Sales"=Picasso[1],"Trend"=Picasso[2],"Cycle"=Picasso[3])
 
 figure_611 <- ggplot(data = table_611, aes(x = Period)) +
  geom_line(aes(y = Sales, color = "Sales")) + 
  geom_line(aes(y = Trend, color = "Trend")) +  
  geom_col(aes(y = z.Cycle, fill = "Cycle")) +        
...
ggdraw(figure_611 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
#ggdraw(switch_axis_position(figure_smooth + theme(panel.background = element_rect(fill = "white", colour = "grey50"), axis = 'y'))
```
...
```{r, fig.cap = "Sales Volum Data, Seasonal Index.", fig.width = 8, fig.height = 4, fig.fullwidth = TRUE,  message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
# chart object
figure_69 <- ggplot(data = table_69_series, aes(y = Sales, x = Period, fill = Series)) +
  geom_bar(position=position_dodge(), stat="identity") + 
  scale_color_manual(values = Picasso) +
  geom_hline(yintercept = 1, linetype="dashed", alpha = 1/2) +
  #scale_y_continuous(breaks=seq(.9,1.3,.05), limit = c(.9,1.3), sec.axis = dup_axis()) +
  coord_cartesian(ylim= c(.9,1.3)) +
  scale_x_continuous(breaks=seq(1,12,1), limits = c(0,13), labels = rep(month.abb))
# draw
ggdraw( figure_69 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
ggdraw( figure_614 + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
```{r, fig.width = 6, fig.height = 3, fig.fullwidth = TRUE, fig.cap = "Sales Volume with DeSeasonalized Data.", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
# Irregular values
...
figure_Irregular_series <- ggplot(data = table_615_Irregular, aes(y = Irregular, x = Period)) +
                                ...
                                geom_hline(yintercept = 1, linetype = "dashed", alpha = 1/2) +
                                scale_x_continuous(breaks=seq(0,54,6), limits = c(0,54)) +
  scale_y_continuous(breaks=seq(0.7,1.1, 0.1), limits = c(0.7,1.1), sec.axis = dup_axis())

# Call graphics Device to illustrate chart
ggdraw(figure_Irregular_series + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
```
...
figure_smooth_c <- ggplot(data = table_smooth_c, aes(x = Period)) +
  geom_line(aes(y = Sales, color = "Sales")) + 
  geom_line(aes(y = Trend, color = "Trend")) +  
  geom_col(aes(y = z.Cycle, fill = "Cycle")) +        
  geom_hline(yintercept = 1, linetype="dashed", alpha = 1/2) +
...
ggdraw(figure_smooth_c + theme(panel.background = element_rect(fill = "white", colour = "grey50"), legend.position = "top", axis.title=element_blank()))
...
```{r, message=FALSE, eval = FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
library(tidyr)
problem_1_long <- rbind(problem_1[,1:2], problem_1[,3:4])
problem_1_long

smooth_problem_1 <- filter(problem_1_long$Observation, c(0.6,0.3,0.1), method = "convolution", sides = 2)
problem_1_series <- data.frame(problem_1_long, "CWMA(3)" = smooth_problem_1)
colnames(problem_1_series) <- c("Period", "Actual", "CWMA(3)")
table_problem_1 <- data.frame(problem_1_series[1:12,], problem_1_series[13:24,])
colnames(table_problem_1) <- c("Period", "Actual", "CWMA(3)", "Period", "Actual", "CWMA(3)")

kable(table_problem_1)
```
