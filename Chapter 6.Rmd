...
Given an economic time series as shown in Figure 6-1 one of the most common approaches to understanding such data is through _classical time series decomposition_. The composition of the time series is decomposed into four components:
trend, cyclical, seasonal, and irregular components.

##The Trend Component

The trend component of a time series at time $t$, denoted $T_t$, is the upward or downward progression of the data over time. Figure 6-10 below has a trend line superimposed over the actual data, indicating a downward trend. The OLS regression line of $T_t$ is defined in the margin^[$$T_t = \beta_{0} + \beta_{1}TIME_t$$].

For our example, the _deterministic trend_ of the Sales Volume series is a simple OLS regression against time period.
...
table_611 <- data.frame(table_61_series, Trend = trend$fitted.values, tCycle = trend$residuals)
...
figure_612 <- ggplot(data = table_611, aes(y = tCycle, x = Period, col = tCycle)) +
  geom_line() + 
...
We shall use the time series from Table 6.2 and Figure 6-1 for this example. Because they are monthly data, we first determine the 12-period Centered Moving Average Smoothing of the data starting with period 7.
...
table_68_seasonal <- data.frame(table_68_seasonal, "DeSeasonalized"= table_68_seasonal$Sales/table_69_adjusted)
kable(table_68_seasonal, digits = 2, caption = "Actual Sales, CWMA(12) Smoothing, Ratio, Seasonal Index, and DeSeasonaled Sales" )
```


Notice in Figure 6-14 that the periods of 11 and 12, 23 and 24, 35 and 36, 47 and
...
table_614_series <- gather(table_68_seasonal[,c(1,3,7)], Period)
colnames(table_614_series) <- c("Period", "Series", "Value")

figure_614 <- ggplot(data = table_614_series, aes(y = Value, x = Period, col = Series)) +
  geom_line() + 
...
Considering the four components collectively is the method of the _classical time series decomposition_. In its multiplicative form, at time $t$, the actual value $Y_t$ is the product >
of the trend component $T_t$, in physical units, the cyclical component $C_t$, as a percentage of $T_t$, the seasonal component $S_t$, as a percentage of $T_t \times C_t$ , and the irregular component, expressed as a percentage of $T_t \times C_t \times S_t$.


**Multiplicative Time Series Decomposition**
$$Y_t = T_t \times C_t \times S_t \times I_t$$.

Time series decomposition may be additive, where all components are expressed in the physical units of $Y_t$. 
...
the scope of this textbook, so we refer the interested reader to the bibliography.
