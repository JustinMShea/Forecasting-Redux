...
plot_62 <- ggplot(data = table_62_series, aes(y = `Horizontal Data`, x = Period)) +
        geom_line(size = 1) +
        ggtitle("Table 6.2: Horizonal Data")
...
```{marginfigure}
$$CMA_{t}(3) = \frac{Y_{T+1}+Y_{T}+Y_{T-1}}{3}$$
```
We shall use the data from Table 6.2 to illustrate this method. For example, for period 7, a 3-period Centered (at period 7) Moving Average is:

$$CMA_{7}(3) = \frac{Y_{8}+Y_{7}+Y_{6}}{3}$$

Specifically,

$$CMA_{7}(3) = \frac{364+367+375{3} = `r round(mean(table_62_series[6:8,2]`$$

```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
## Smoothing moving average, pg 8.
period_3_example <- table_62_series[6:8,]
smooth <- round(mean(period_3_example[,2]), digits = 2)
period_3_example$Smoothing <- c("...",smooth,"...")


kable(period_3_example)
```
