...
Because the $Y_t$ are varying around a constant mean, a first smoothing of $Y_t$ is just its mean, $\bar{Y}$.^[$$\bar{Y} = \sum_{t=1}^{T} Y_{t}$$] In this example, $\bar{Y} = `r round(mean(table_62_series[,2]), digits = 1)`$.
...
kable(table_69_ratio, caption = "Ratios of Actual Sales to CWMA(12) Smoothing")
```


```{r, fig.cap = "Unadjusted Seasonal Index - Median Value", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
kable(table_69_median_tbl, caption = "Unadjusted Seasonal Index - Median Value" )
```
...
The sum of the Adjusted Seasonal Indices equals 12, so their average equals 1. As expected, the 12th month, December, has the largest seasonal index, and the 7th month, July, has the smallest seasonal index.
...
```{r, fig.cap = "Sales Volum Data, Seasonal Index.",fig.width = 10, fig.height = 4, fig.fullwidth = TRUE,  message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
Notice in Figure 6-15 that the periods of 11 and 12, 23 and 24, 35 and 36, 47 and
48 are reoccurring local peaks in Sales Volume. This is the seasonality of Sales.
...
```{r, fig.width = 10, fig.height = 3, fig.fullwidth = TRUE, fig.cap = "DeSeasonalized Sales Volume Data, Cycle component.", message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
6-13 are the random, unexpected deviations from the trend/cyclical/seasonal components.
...
In Figure 6-13 below, an irregular variation occurs around period 30 as the series drops abruptly and then returns to its upward trend.
...
# Create Series for chart
table_Irregular_series <- gather(table_614_series[,c(1,3,7,8)], Period)
colnames(table_614_series2) <- c("Period", "Series", "Value")
