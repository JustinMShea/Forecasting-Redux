...
kable(period_3_example, align = "l", row.names=FALSE, caption="Horizontal Data Table 6.2 subset: Centered Smoothing Average (3)")
```
...
table_63 <- data.frame(table_62_series, "CMA(3)" = smooth_63)
colnames(table_63) <- c("Period", "Actual", "Centered Moving Average (3)")
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = table_63, aes(y = `Actual`, x = Period)) +
        geom_line(color = "blue", size = 1) +
        ggtitle("3-Period Centered Moving Average") +
        geom_line(aes(y = `Centered Moving Average (3)`), color = "red", linetype="dashed", size = 1.3)
```
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE, fig.margin = TRUE}
kable(period_5_example, align = "l", row.names=FALSE, caption="Horizontal Data Table 6.3 subset: Centered Smoothing Average (5)")
```

   We list below both the 3-period and 5-period moving average smoothing and a graph of the actual values with the two smoothings superimposed.
   
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE, fig.margin = TRUE}
## creating table 6.4
library(forecast)
smooth_64 <- round(ma(table_62_series[,2], order = 5, centre = TRUE), digits = 1)
table_64 <- data.frame(table_62_series, "CMA(3)" = smooth_63, "CMA(5)" = smooth_64)
colnames(table_64) <- c("Period", "Actual", "CMA(3)", "CMA(5)")
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = table_64, aes(y = `Actual`, x = Period)) +
        geom_line(color = "blue", size = 1) +
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = table_64, aes(y = `Actual`, x = Period))+ geom_line(color = "darkgreen", size = 1) +
geom_line(aes(y = `CMA(3)`), color = "red", linetype="dashed", size = 1.4) +
geom_line(aes(y = `CMA(5)`), color = "blue", linetype="dashed", size = 1.4) + 
        ggtitle("3-Period and 5-Period Centered Moving Average") 
```


## 4-Period Centered Moving Average Smoothing

A 4-period Centered Moving Average is possible, but the issue of the placing of the results remains.^[4-period Centered Moving Average: $$CMA_{t}(4) = \frac{Y_{T+2}+2Y_{T+1}+2Y_{T}+2Y_{T-1}+Y_{T-2}}{8}$$] Technically, if (say) the first 4 periods are used, 
...
Then the "adjusted centered smoothing" for period 3 is the average of period 2.5 and period 3.5 smoothing.

$CMA_{t}(4) = \frac{360+365.25}{2} = 362.63$

Algebraically, it can be shown that the adjusted moving average of 4 periods, starting with period $t = 3$ is shown on the margin.
...
kable(table_65, align = "l", row.names=FALSE, caption="Table 6.5: 
Centered Smoothing Average (4)")
```

```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
ggplot(data = table_65, aes(y = `Actual`, x = Period)) +
        geom_line(color = "red", size = 1) +
        ggtitle("4-Period Centered Moving Average") +
        geom_line(aes(y = `CMA(4)`), color = "blue", linetype="dashed", size = 1.4)
```
