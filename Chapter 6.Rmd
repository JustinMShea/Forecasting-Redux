...
colnames(problem_4) <-c("Period($t$)","Quarter($Q_t$)","Actual($Y_t$)","Period($t$)","Quarter($Q_t$)","Actua($Y_t$)")
library(xtable)
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
## Import data from problem text file
problem_8_path <- path.expand("~/R/forecasting/problem 8 table.txt")
problem_8 <- read.delim(problem_8_path, skip = 1, sep = " ")
# add column names
