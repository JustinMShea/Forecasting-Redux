...
Becuase the $Y_t$ are varying around a constant mean, a first smoothing of $Y_t$ is just its mean, $\bar{Y}$.^[Smoothing using the sample mean: $$\bar{Y} = \sum_{t=1}^{T} Y_{t}$$]
...
```{r, fig.margin = TRUE, fig.width=3.5, fig.height=3.5, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE, fig.width = 10, fig.height = 4, fig.fullwidth = TRUE}
library(forecast)
smooth_64 <- round(ma(table_62_series[,2], order = 5, centre = TRUE), digits = 1)
table_64_series <- data.frame(table_62_series, "CMA(3)" = smooth_63, "CMA(5)" = smooth_64)
colnames(table_64_series) <- c("Period", "Actual", "CMA(3)", "CMA(5)")
...
chart_65 <- ggplot(data = table_65_chart, aes(y = Value, x = Period, col = Series)) +
            geom_line(position = "jitter") +
            ggtitle("Actual ~ CMA(4)")  +
            theme(legend.position = 'bottom')

chart_65
#figure_65 <-ggplot(data = table_65, aes(y = `Actual`, x = Period)) +
 #       geom_line(color = "red", size = 1) +
  #      ggtitle("Actual ~ CMA(4)") +
   #     geom_line(aes(y = `CMA(4)`), color = "blue", linetype="dashed", size = 1.4)

#ggdraw(switch_axis_position(chart_65 + theme_tufte(), axis = 'y'))
```
