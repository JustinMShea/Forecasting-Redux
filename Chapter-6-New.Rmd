...
author: "Author: Joel Fingerman"
date: "Markdown complined by Justin M. Shea"
output:
  tufte::tufte_handout:  
...
Table_61_path <- paste0(getwd(),"/data/Table 6.1.txt")
...
Table_62_path <- paste0(getwd(),"/data/Table 6.2.txt")
...
figure_621 <- ggplot(data = table_62_series, aes(y = `Horizontal Data`, x = Period)) +
        geom_line() + 
        scale_x_continuous(breaks=seq(300,400,10), limits = c(300,400), sec.axis = dup_axis()) +
...
## 3-period Centered Moving Average Smoothing for period $t$ ^[3-period Centered Moving Average: $$CMA_{t}(3) = \frac{Y_{T+1}+Y_{T}+Y_{T-1}}{3}$$ $$CMA_{7}(3) = \frac{Y_{8}+Y_{7}+Y_{6}}{3}$$ $$CMA_{7}(3) = \frac{364+367+375}{3}=368.66$$]


To begin with an example, examine the notation in the margin. Next, we shall use the data from Table 2 to illustrate this method. For example, a 3-period Centered Moving Average (at period 7) is:
...
```{r, fig.cap = "Centered Moving Average(3)", fig.width=4, fig.height=3.5, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
...
ggdraw(chart_63 + theme_tufte() + theme(legend.position = "top"))
```
...
ggdraw(chart_641 + theme_tufte() + theme(legend.position = "bottom"))
```
...
ggdraw(chart_642 + theme_tufte() + theme(legend.position = "bottom"))
```
...
ggdraw(chart_65 + theme_tufte() + theme(legend.position = "top"))
```
\medskip
...
ggdraw(chart_66 + theme_tufte() + theme(legend.position = "bottom"))
```
...
ggdraw( chart_67 + theme_tufte() + theme(legend.position = "top") )
```
...
problem_1_path <- paste0(getwd(),"/data/problem table 1.txt")
...
problem_4_path <- paste0(getwd(),"/data/problem 4 table.txt")
...
```{r, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE, echo=FALSE}
## Import data from problem text file
problem_8_path <- paste0(getwd(),"/data/problem 8 table.txt")
